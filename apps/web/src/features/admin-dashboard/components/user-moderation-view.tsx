...
import { Users, Plus, Key, Ban, ShieldAlert, Search, UserCheck, ShieldCheck } from "lucide-react";
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from "@engducation/ui/components/select";
import {
  Table,
  TableBody,
  TableCell,
...
interface UserModerationViewProps {
  /** The admin's own user ID — used to prevent self-ban */
  adminId: string;
}

export function UserModerationView({ adminId }: UserModerationViewProps) {
  const {
    users,
    isLoading,
    search,
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    createUser({ name: newUserName, email: newUserEmail, password: newUserPassword, role: newUserRole });
  ...
  const handleConfirmBan = () => {
    ...
    banUser(selectedUserId, banReason);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-violet-100 text-xs md:text-sm mt-0.5 font-light">
                Xem danh sách thành viên đang hoạt động, tạm khóa hoặc nâng cấp/khởi tạo
                quản trị viên trực tiếp bằng Better-Auth.
              </p>
            </div>
          </div>
          ...
          <Dialog open={createUserOpen} onOpenChange={setCreateUserOpen}>
            ...
            <DialogContent className="max-w-md bg-card rounded-2xl border">
              <DialogHeader>
                ...
                <DialogDescription>
                  Tạo trực tiếp tài khoản mới vào hệ thống. Mật khẩu sẽ được mã hóa an
                  toàn qua Better-Auth.
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleCreateUser} className="space-y-4 py-2">
                <div className="space-y-2">
                  <Label htmlFor="mod-user-name">Họ và tên *</Label>
                  <Input
                    id="mod-user-name"
                ...
                <div className="space-y-2">
                  <Label htmlFor="mod-user-email">Địa chỉ Email *</Label>
                  <Input
                    id="mod-user-email"
                    type="email"
                ...
                <div className="space-y-2">
                  <Label htmlFor="mod-user-password">Mật khẩu *</Label>
                  <div className="flex gap-2">
                    <Input
                      id="mod-user-password"
                      type="text"
                ...
                <div className="space-y-2">
                  <Label htmlFor="mod-user-role">Vai trò trên hệ thống *</Label>
                  <Select
                    value={newUserRole}
                ...
                <DialogFooter className="pt-4">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => { setCreateUserOpen(false); resetCreateForm(); }}
      ...
      <Dialog open={banDialogOpen} onOpenChange={setBanDialogOpen}>
        <DialogContent className="max-w-md bg-card rounded-2xl border">
          <DialogHeader>
            ...
            <DialogDescription>
              Hành động này sẽ lập tức hủy bỏ toàn bộ phiên làm việc (Session/Token) của
              người dùng đó trên tất cả các thiết bị. Người dùng sẽ bị đẩy ra khỏi hệ
              thống ngay lập tức.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <Label htmlFor="ban-reason-mod">Lý do khóa tài khoản *</Label>
            <Textarea
              id="ban-reason-mod"
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => { setBanDialogOpen(false); setSelectedUserId(null); setBanReason(""); }}
              className="rounded-xl"
            >
              Hủy
            </Button>
            <Button
              ...
              disabled={isBanning}
            >
              {isBanning ? "Đang xử lý..." : "Khóa ngay lập tức"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
